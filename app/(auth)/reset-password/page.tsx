    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto flex min-h-screen max-w-md items-center px-4">
        <div className="w-full rounded-3xl border bg-card p-6 shadow-sm sm:p-8">
          ...
          {!token ? (
            <div className="space-y-4 rounded-2xl border border-destructive/20 bg-destructive/10 p-4 text-sm text-destructive">
              <p>الرابط غير صالح.</p>
              <Link
            ...
            <form action={formAction} className="space-y-4">
              ...
              {state?.success ? (
                <div className="rounded-2xl border border-primary/20 bg-primary/5 px-3 py-2 text-sm text-foreground">
                  {state.message}
                </div>
              ) : null}
